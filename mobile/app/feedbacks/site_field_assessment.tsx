...
export default function SiteFieldAssessment() {
  // ✅ Extract siteId and siteName to handle Specific vs General context
  ...
  const fetchAssessments = async () => {
    try {
      ...
      // Omit it entirely for general assessments to avoid Django integer casting errors.
      if (siteId) {
        url += `&site_id=${siteId}`;
      }
  ...
  // ✅ FIXED: Check nested field_assessment_data instead of non-existent 'a.layer'
  const getStatus = (layerId: string): LayerStatus => {
    const relevantAssessments = assessments.filter((a) => {
      ...
      return layerData && typeof layerData === 'object' && Object.keys(layerData).length > 0;
    });

    if (relevantAssessments.some((a) => a.is_submitted)) return "done";
    if (relevantAssessments.length > 0) return "draft";
    return "pending";
  };
  ...
  const getCount = (layerId: string, submitted: boolean) => {
    return assessments.filter((a) => {
      const layerData = a.field_assessment_data?.[layerId];
      const hasData = layerData && typeof layerData === 'object' && Object.keys(layerData).length > 0;
  ...
  const headerSub = siteId ? "Specific Site Assessment" : "General Area Assessment";
    ...
    <View style={styles.root}>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={[styles.content, { paddingBottom: insets.bottom + 32 }]} showsVerticalScrollIndicator={false}>
        <View style={styles.progressCard}>
          <View style={styles.progressRow}>
            <View>
              ...
              <Text style={styles.progressSub}>{completedCount} of {LAYERS.length} layers submitted</Text>
            </View>
            ...
            {LAYERS.map((l) => {
              ...
              return (
                <View key={l.id} style={styles.dotItem}>
                  <View style={[styles.dot, s === "done" && { backgroundColor: "#5FD08A" }, s === "draft" && { backgroundColor: "#F59E0B" }, s === "pending" && { backgroundColor: "rgba(255,255,255,0.25)" }]} />
        ...
        <View style={styles.sectionRow}>
          ...
          <View style={styles.countBadge}><Text style={styles.countText}>{LAYERS.length} Layers</Text></View>
        </View>
        ...
        <View style={styles.grid}>
          {LAYERS.map((layer) => {
            ...
            return (
              <TouchableOpacity
                key={layer.id}
                style={[styles.card, status === "draft" && styles.cardDraft, status === "done" && styles.cardDone]}
              ...
              >
                ...
                <View style={styles.cardInner}>
                  ...
                  <View style={styles.cardFooter}>
                    ...
                    {status === "pending" && <Text style={styles.pendingText}>Tap to start</Text>}
        ...
        <View style={styles.infoBanner}>
          <View style={styles.infoBannerIcon}><MaterialCommunityIcons name="information-outline" size={18} color="#0F4A2F" /></View>
          <Text style={styles.infoText}>
            {siteId ? "Complete all four layers for this specific site." : "Complete all four layers for this general area."}
          </Text>
        </View>
      </ScrollView>
    </View>
...
const styles = StyleSheet.create({
  ...
  scroll: { flex: 1 }, content: { padding: 16 },
  progressCard: { backgroundColor: "#0F4A2F", borderRadius: 18, padding: 18, marginBottom: 20 },
  progressRow: { flexDirection: "row", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 },
  ...
  progressBg: { height: 6, backgroundColor: "rgba(255,255,255,0.18)", borderRadius: 3, overflow: "hidden", marginBottom: 14 },
  ...
  countBadge: { backgroundColor: "#E6F4EC", paddingHorizontal: 8, paddingVertical: 2, borderRadius: 10 },
  countText: { fontSize: 11, color: "#0F4A2F", fontWeight: "700" },
  grid: { flexDirection: "row", flexWrap: "wrap", gap: 12, marginBottom: 16 },
  card: { width: "47%", backgroundColor: "#FFFFFF", borderRadius: 16, overflow: "hidden", elevation: 2, shadowColor: "#000", shadowOpacity: 0.05, shadowRadius: 6, shadowOffset: { width: 0, height: 2 }, borderWidth: 1, borderColor: "#F3F4F6" },
  cardDraft: { borderColor: "#FCD34D", borderWidth: 1.5, backgroundColor: "#FFFBEB" },
  cardDone: { borderColor: "#86EFAC", borderWidth: 1.5, backgroundColor: "#F0FDF4" },
  cardAccent: { height: 4, width: "100%" },
  cardInner: { padding: 14 },
  iconCircle: { width: 46, height: 46, borderRadius: 14, justifyContent: "center", alignItems: "center", marginBottom: 10 },
  cardLabel: { fontSize: 13, fontWeight: "700", color: "#0F2D1C", marginBottom: 8, lineHeight: 18 },
  cardFooter: { flexDirection: "row", alignItems: "center", justifyContent: "space-between" },
  badgeDone: { flexDirection: "row", alignItems: "center", gap: 3, backgroundColor: "#DCFCE7", paddingHorizontal: 7, paddingVertical: 3, borderRadius: 10 },
  badgeDoneText: { fontSize: 10, color: "#15803D", fontWeight: "700" },
  badgeDraft: { flexDirection: "row", alignItems: "center", gap: 3, backgroundColor: "#FEF3C7", paddingHorizontal: 7, paddingVertical: 3, borderRadius: 10 },
  badgeDraftText: { fontSize: 10, color: "#92400E", fontWeight: "700" },
  pendingText: { fontSize: 11, color: "#9CA3AF", fontStyle: "italic" },
  infoBanner: { flexDirection: "row", alignItems: "flex-start", gap: 10, backgroundColor: "#E6F4EC", borderRadius: 12, padding: 14 },
  infoBannerIcon: { width: 30, height: 30, borderRadius: 10, backgroundColor: "#FFFFFF", justifyContent: "center", alignItems: "center", flexShrink: 0 },
