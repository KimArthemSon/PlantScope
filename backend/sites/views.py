...
def create_site(request):
    ...
    try:
        ...
        if not marker_coordinate and polygon_coordinates and len(polygon_coordinates) >= 3:
            ...
            marker_coordinate = [round(sum_lat / len(polygon_coordinates), 6), round(sum_lng / len(polygon_coordinates), 6)]
...
def get_sites(request, reforestation_area_id):
    ...
    if all_filter == "true":
        sites = Sites.objects.filter(is_active=True).select_related(
...
def get_site(request, site_id):
    ...
    if verification:
        verified_animals = [
            {"animal_id": rel.animal.animal_id, "name": rel.animal.name, "scientific_name": rel.animal.scientific_name, "admin_notes": rel.admin_notes}
...
def toggle_pin(request, site_id):
    ...
    try:
        ...
        site.save()
        return JsonResponse({"message": f"Site {'pinned' if site.is_pinned else 'unpinned'} successfully.", "is_pinned": site.is_pinned}, status=200)
    except Exception as e:
    ...
    try:
        ...
        with transaction.atomic():
            ...
            for i, sp in enumerate(species_list):
                ...
                if not tree_species_id: continue
                tree_species = get_object_or_404(Tree_species, tree_specie_id=tree_species_id)
                Site_species_recommendation.objects.create(
                    site=site, tree_species=tree_species, priority_rank=sp.get("priority_rank", i + 1), notes=sp.get("notes", "")
                )
...
def get_sites_list(request, reforestation_area_id):
    ...
    sites = Sites.objects.filter(reforestation_area_id=reforestation_area_id, is_active=True).select_related('meta_verification').order_by("-is_pinned", "-created_at")
    ...
    for s in sites:
        ...
        validation = {"has_safety_note": False, "has_survivability_note": False, "final_decision": None}
        
        if latest_sd and latest_sd.site_data:
            validation["has_safety_note"] = bool(latest_sd.site_data.get('safety', {}).get('decision_note', '').strip())
            validation["has_survivability_note"] = bool(latest_sd.site_data.get('survivability', {}).get('decision_note', '').strip())
...
def save_validation_draft(request, site_id):
    ...
    try:
        ...
        if 'safety' in body and 'decision_note' in body['safety']:
            if 'safety' not in sd.site_data: sd.site_data['safety'] = {}
        ...
        if 'survivability' in body and 'decision_note' in body['survivability']:
            if 'survivability' not in sd.site_data: sd.site_data['survivability'] = {}
...
def finalize_site(request, site_id):
    ...
    try:
        ...
        if final_decision not in ["ACCEPT", "REJECT"]:
            return JsonResponse({"error": "final_decision must be 'ACCEPT' or 'REJECT'"}, status=400)
...
def delete_site(request, site_id):
    ...
    try:
        ...
        site.delete()
        return JsonResponse({"message": "Site permanently deleted", "images_deleted": deleted_images})
    except Exception as e:
...
def _serialize_assessment(assessment, assessment_type):
    ...
    if user:
        ...
        inspector_name = f"{first} {last}".strip() or getattr(user, 'email', 'Unknown') or getattr(user, 'username', 'Unknown')

    inspector_profile_img = None
    if user:
        profile = getattr(user, 'profile', None)
        if profile and profile.profile_img:
            try: inspector_profile_img = get_cloudinary_url(str(profile.profile_img))
            except: pass
    ...
    if assessment.land_classification:
        land_classification_data = {'id': assessment.land_classification.land_classification_id, 'name': assessment.land_classification.name}
    
    animals_present_data = []
    try:
        for rel in assessment.animal_relations.select_related('animal').all():
            animals_present_data.append({'animal_id': rel.animal.animal_id, 'name': rel.animal.name, 'scientific_name': rel.animal.scientific_name or ''})
    except Exception as e:
...
def get_site_verification(request, site_id):
    ...
    site = get_object_or_404(Sites, site_id=site_id, is_active=True)
    verification, _ = SiteMetaDataVerification.objects.get_or_create(site=site, defaults={'status': 'pending'})
    
    assessments_data = []
    if Field_assessment is not None:
        try:
            specific_assessments = Field_assessment.objects.filter(site=site, is_submitted=True, field_assessment_data__has_key='meta_data').select_related('assigned_onsite_inspector', 'assigned_onsite_inspector__user', 'land_classification').prefetch_related('images', 'animal_relations__animal').order_by('-assessment_date')
            for a in specific_assessments: assessments_data.append(_serialize_assessment(a, 'specific'))
            
            general_assessments = Field_assessment.objects.filter(site__isnull=True, assigned_onsite_inspector__reforestation_area=site.reforestation_area, is_submitted=True, field_assessment_data__has_key='meta_data').select_related('assigned_onsite_inspector', 'assigned_onsite_inspector__user', 'assigned_onsite_inspector__reforestation_area', 'land_classification').prefetch_related('images', 'animal_relations__animal').order_by('-assessment_date')
            for a in general_assessments: assessments_data.append(_serialize_assessment(a, 'general'))
        except Exception as e:
            logger.warning(f"Could not fetch field assessments: {e}")

    sec_concerns = list(verification.verified_security_concerns) if isinstance(verification.verified_security_concerns, set) else (verification.verified_security_concerns or [])
    acc = list(verification.verified_accessibility) if isinstance(verification.verified_accessibility, set) else (verification.verified_accessibility or {})

    verified_animals = []
    try:
        for rel in verification.animal_relations.select_related('animal').all():
            verified_animals.append({'animal_id': rel.animal.animal_id, 'name': rel.animal.name, 'scientific_name': rel.animal.scientific_name or '', 'admin_notes': rel.admin_notes or ''})
    except Exception as e:
        logger.warning(f"Could not fetch verified animals: {e}")

    return JsonResponse({
        'verification': {
            'id': verification.id, 'status': verification.status, 'verified_security_concerns': sec_concerns,
            'verified_accessibility': acc, 'verified_land_classification_id': verification.verified_land_classification_id,
            'verified_land_classification_name': verification.verified_land_classification.name if verification.verified_land_classification else None,
            'decision_note': verification.decision_note or '', 'referenced_assessment_ids': verification.referenced_assessment_ids or [],
            'verified_by': verification.verified_by.email if verification.verified_by else None,
            'verified_at': verification.verified_at.isoformat() if verification.verified_at else None,
            'verified_animals': verified_animals,
        },
...
def update_site_verification(request, site_id):
    ...
    try:
        ...
        verification, created = SiteMetaDataVerification.objects.get_or_create(site=site, defaults={'status': 'draft', 'verified_by': user})

        if 'verified_security_concerns' in data: verification.verified_security_concerns = data['verified_security_concerns']
        if 'verified_accessibility' in data: verification.verified_accessibility = data['verified_accessibility']
        if 'verified_land_classification_id' in data: verification.verified_land_classification_id = data['verified_land_classification_id']
        if 'decision_note' in data: verification.decision_note = data['decision_note']
        if 'status' in data: verification.status = data['status']
        if 'referenced_assessment_ids' in data: verification.referenced_assessment_ids = data['referenced_assessment_ids']
        ...
        if 'verified_animals' in data:
            ...
            if not isinstance(verified_animals, list):
                return JsonResponse({'error': 'verified_animals must be an array'}, status=400)
            ...
            with transaction.atomic():
                ...
                for animal_data in verified_animals:
                    ...
                    try:
                        ...
                        SiteVerifiedAnimal.objects.create(verification=verification, animal=animal, admin_notes=animal_data.get('notes', ''))
                    except Animal.DoesNotExist:
                        logger.warning(f"Animal {animal_id} not found, skipping.")
                        continue
        
        return JsonResponse({'message': f'Verification {verification.status}', 'status': verification.status, 'verification_id': verification.id}, status=200)
    except Exception as e:
...
def list_site_permits(request, site_id):
    ...
    data = [{'permit_id': p.permit_id, 'document_type': p.document_type, 'notes': p.notes, 'verification_notes': p.verification_notes, 'uploaded_at': p.uploaded_at.isoformat(), 'uploaded_by': p.uploaded_by.email if p.uploaded_by else None} for p in permits]
...
def upload_site_permit(request, site_id):
    ...
    try:
        permit = PermitDocument.objects.create(site=site, document_type=document_type, notes=notes or None, verification_notes=verification_notes or None, uploaded_by=user)
        return JsonResponse({'message': 'Permit record created', 'permit_id': permit.permit_id}, status=201)
    except Exception as e:
...
def update_site_coordinates(request, site_id):
    ...
    try:
        ...
        if 'polygon_coordinates' in body:
            ...
            if not isinstance(new_polygon, list) or len(new_polygon) < 3:
                return JsonResponse({"error": "polygon_coordinates must be an array with at least 3 points"}, status=400)
            
            for i, coord in enumerate(new_polygon):
                if not isinstance(coord, (list, tuple)) or len(coord) != 2:
                    return JsonResponse({"error": f"Invalid coordinate at index {i}. Expected [lat, lng]"}, status=400)
                try:
                    lat, lng = float(coord[0]), float(coord[1])
                    if not (4 <= lat <= 21) or not (117 <= lng <= 127):
                        return JsonResponse({"error": f"Invalid coordinate at index {i}: [{lat}, {lng}]"}, status=400)
                except (ValueError, TypeError):
                    return JsonResponse({"error": f"Invalid coordinate values at index {i}"}, status=400)
        ...
        if 'marker_coordinate' in body:
            ...
            if not isinstance(new_marker, (list, tuple)) or len(new_marker) != 2:
                return JsonResponse({"error": "marker_coordinate must be [lat, lng]"}, status=400)
            
            try:
                lat, lng = float(new_marker[0]), float(new_marker[1])
                if not (4 <= lat <= 21) or not (117 <= lng <= 127):
                    return JsonResponse({"error": f"Invalid marker coordinate: [{lat}, {lng}]"}, status=400)
            except (ValueError, TypeError):
                return JsonResponse({"error": "Invalid marker coordinate values"}, status=400)
            
            site.marker_coordinate = new_marker
            marker_updated = True
            logger.info(f"Site {site_id} marker updated: [{lat}, {lng}]")
        
        if polygon_updated and not marker_updated:
            site.marker_coordinate = site.calculate_marker_coordinate()
            marker_updated = True
            logger.info(f"Site {site_id} marker auto-calculated from polygon: {site.marker_coordinate}")
        
        if polygon_updated or marker_updated:
            site.save()
            return JsonResponse({
            ...
            return JsonResponse({"error": "No coordinates provided to update"}, status=400)
    ...
    try:
        ...
        return JsonResponse({
            ...
            "reforestation_area": {
                ...
                "barangay": {"barangay_id": area.barangay.barangay_id, "name": area.barangay.name} if area.barangay else None
            },
            "sites": sites_data
        }, status=200)
    except Exception as e:
