...
// ✅ Normalize coordinates to ensure [lat, lng] format for polygons
...
// ✅ Normalize single marker coordinates to ensure [lat, lng] format
...
const createMarkerIcon = (
  ...
  return L.divIcon({
    ...
    iconAnchor: [12, 30], // ✅ CRITICAL FIX: Always anchor to the tip of the 24px wide pin
    popupAnchor: [0, -30],
  });
};
...
export default function MulticriteriaAnalysis() {
  ...
  // ✅ NEW: State for Coordinate Probe Mode
  const [isCoordinateProbeMode, setIsCoordinateProbeMode] = useState(false);

  // ✅ NEW: State for live coordinate display
  ...
  // ✅ NEW: State for showing/hiding coordinates in field assessment popups
  const [showAssessmentCoordinates, setShowAssessmentCoordinates] =
    useState(true);

  // ✅ UPDATED: Pass showAssessmentCoordinates to the hook
  const fieldAssessments = useFieldAssessments(
    mapRef,
    showAssessmentCoordinates,
  );
  ...
  // ✅ UPDATED: Map click handler with "Pick from Map" and "Coordinate Probe" features
  useEffect(() => {
    const map = mapRef.current;
    if (!map) return;
    const handleClick = (e: L.LeafletMouseEvent) => {
      ...
      } else if (isPlacingNewMarker) {
        handleMapClickForNewMarker(e);
      ...
      <main className="flex-1 p-3 flex flex-col gap-3">
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 px-3 py-2 flex items-center justify-between">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {/* ✅ NEW: Coordinate Probe Toggle Button */}
            <button
                      ...
                      onClick={() => {
                        ...
                        if (newState) {
                          setAlert({
                            ...
                            message:
                              "Click anywhere on the map to place the site marker.",
                          });
                        }
                      }}
        ...
        <div className="flex gap-3 flex-1 min-h-0">
          ...
          <div className="flex-[2] flex flex-col gap-3 min-w-0">
            <div className="flex-1 bg-white rounded-lg border border-gray-200 flex flex-col min-h-0">
              {/* ✅ UPDATED: Field Assessments Header with Toggle Button */}
              <div className="px-3 py-2 border-b border-gray-100 flex items-center justify-between flex-shrink-0">
                ...
                <div className="flex items-center gap-2">
                  <button
                    onClick={() =>
                      setShowAssessmentCoordinates(!showAssessmentCoordinates)
                    }
                    className={`text-[10px] font-medium flex items-center gap-1 px-2 py-1 rounded transition border ${
                      showAssessmentCoordinates
                    ...
                    title={
                      showAssessmentCoordinates
                        ? "Hide coordinates in popups"
                        : "Show coordinates in popups"
                    }
                  >
                    <MapPin size={10} />
                    {showAssessmentCoordinates ? "Hide Coords" : "Show Coords"}
                  </button>
                  {areaId && (
                    <button
                      onClick={() =>
