...
export default function ReforestationAreaSiteCombined() {
    ...
    const fetchDropdowns = async () => {
      try {
        const [bRes, lcRes] = await Promise.all([
          ...
          fetch(`${api}api/get_land_classifications_list/`, {
            headers: { Authorization: `Bearer ${token}` },
          }),
        ]);
  ...
  const fetchSites = async () => {
    ...
    try {
      ...
      let url = "";
      if (selectedArea) {
        url = `${api}api/get_sites/${selectedArea.reforestation_area_id}/?${params}`;
      } else {
        params.append("all", "true");
        url = `${api}api/get_sites/0/?${params}`;
      }
  ...
  // Triggers for Parent (Areas) with Debounce
  useEffect(() => {
    ...
    areaFilter.page,
    areaFilter.entries,
    areaFilter.barangay_id,
    areaFilter.search,
  ]);

  // Triggers for Child (Sites) with Debounce
  useEffect(() => {
    const timer = setTimeout(() => {
      fetchSites();
    }, 400);
  ...
  const hasActiveFilters =
    siteFilter.verification_status !== "all" ||
    siteFilter.land_classification_id !== "" ||
    siteFilter.pinned_only;
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-green-200">
                    Area #{viewArea.reforestation_area_id}
                  </p>
                </div>
              </div>
              <button
                      ...
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <p className="text-2xl font-bold text-blue-800 mt-0.5">
                            {areaDetails.total_area_hectares.toFixed(1)}{" "}
                      ...
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <p className="text-2xl font-bold text-purple-800 mt-0.5">
                            {areaDetails.total_seedlings.toLocaleString()}
                          </p>
                        </div>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                    <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm">
                      <h3 className="text-sm font-semibold text-slate-800 mb-4 flex items-center gap-2">
                        <PieChart size={16} className="text-[#0F4A2F]" /> Status
                        Distribution
                      </h3>
                      <div className="h-56">
                        <ResponsiveContainer width="100%" height="100%">
                          <RePieChart>
                            <Pie
                              ...
                              labelLine={false}
                            >
                              {areaDetails.status_distribution.map(
                                (entry, index) => (
                                  <Cell
                                    key={`cell-${index}`}
          ...
          <div className="w-96 flex flex-col bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
            ...
            <div className="p-2.5 border-t border-slate-200 bg-slate-50/50 flex items-center justify-between">
              <button
                ...
                onClick={() =>
                  setAreaFilter((prev) => ({ ...prev, page: prev.page - 1 }))
                }
          ...
          <div className="flex-1 flex flex-col bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
            <div className="p-4 border-b border-slate-200 bg-slate-50/50">
              <div className="flex items-center justify-between mb-1">
                ...
                {/* Add New Site ONLY shows if a specific area is selected */}
                {userRole !== "DataManager" && selectedArea && (
                  <button
                    onClick={() =>
                      navigate(
              ...
              <p className="text-xs text-slate-500 ml-9">
                {selectedArea
                  ? "Manage and verify reforestation sites in this area"
                  : "Manage and verify sites across all reforestation areas"}
              </p>
            </div>
                  ...
                  <div className="absolute right-0 top-full mt-2 w-72 bg-white rounded-xl shadow-xl border border-slate-200 p-4 z-50 animate-in fade-in slide-in-from-top-2 duration-200">
                    <div className="space-y-4">
                      ...
                      <div className="flex items-center gap-2 pt-2 border-t border-slate-100">
                        ...
                        <label
                          htmlFor="pinned-only"
                          className="text-sm text-slate-700 cursor-pointer select-none flex items-center gap-1.5"
                        >
                          <Pin
                            size={14}
                            className={
                  ...
                  {sites.length > 0 ? (
                    sites.map((site, index) => {
                      ...
                      return (
                        <tr
                          key={site.site_id}
                          className={`hover:bg-slate-50 transition-colors ${index % 2 ? "bg-slate-50/30" : "bg-white"} ${site.is_pinned ? "border-l-2 border-emerald-500 bg-emerald-50/40" : ""}`}
                        >
                          <td className="py-2.5 px-3 text-xs text-slate-600">
                            {index +
                              1 +
                              (siteFilter.page - 1) * siteFilter.entries}
                          </td>
                          <td className="py-2.5 px-3">
                            <div className="flex items-center gap-1.5">
                              <button
                                ...
                                className={`p-0.5 rounded hover:bg-slate-200 transition ${site.is_pinned ? "text-emerald-600" : "text-slate-400"}`}
                              >
                                {site.is_pinned ? (
                          ...
                          <td className="py-2.5 px-3">
                            <span
                              className={`px-2 py-1 rounded-full text-[10px] font-medium ${getStatusColor(site.status)}`}
                            >
                          ...
                          <td className="py-2.5 px-3">
                            <div className="flex gap-1">
                              ...
                              {/* Analysis Button - ONLY shows in "All Areas" global view */}
                              {!selectedArea && userRole !== "DataManager" && (
                                <button
                                  onClick={() =>
                                    navigate(
                                      `${userPath}/analysis/multicriteria-analysis/new?areaId=${site.reforestation_area_id}`,
                                    )
                                  }
                                  className="p-1.5 text-purple-600 hover:bg-purple-50 rounded-md transition-colors border border-purple-600"
                                  title="Run Analysis (MCDA)"
                                >
                                  <Target size={12} />
                                </button>
                              )}
            ...
            <div className="p-2.5 border-t border-slate-200 bg-slate-50/50 flex items-center justify-between">
              <button
                ...
                onClick={() =>
                  setSiteFilter((prev) => ({ ...prev, page: prev.page - 1 }))
                }
                className="p-1.5 border border-slate-300 rounded-md disabled:opacity-50 hover:bg-white transition-colors"
              >
                <ChevronLeft size={14} />
              ...
              <div className="flex gap-1">
                {Array.from(
                  { length: Math.min(siteFilter.total_page, 5) },
                  (_, i) => {
                    let pageNum = i + 1;
                    if (siteFilter.total_page > 5 && siteFilter.page > 3) {
                      pageNum = Math.min(
                        siteFilter.page - 2 + i,
                        siteFilter.total_page,
                      );
                    }
                    return (
                      <button
                        key={pageNum}
                        onClick={() =>
                          setSiteFilter((prev) => ({ ...prev, page: pageNum }))
                        }
