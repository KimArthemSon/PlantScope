...
function getAcceptanceColor(acceptance?: string) {
  ...
  if (acceptance.startsWith("ACCEPT_WITH"))
    return {
      bg: "bg-yellow-100",
...
const ValidationProgressCard: React.FC<{ mcda: SiteData }> = ({ mcda }) => {
  const { validated_layers, total_layers, rejected_layers, is_complete } =
    computeValidationProgress(mcda);
  const percent =
    total_layers > 0 ? (validated_layers / total_layers) * 100 : 0;
      ...
      <div className="px-5 pt-4 pb-2">
        <div className="flex justify-between items-center mb-1.5">
          <span className="text-xs text-gray-500">
            {validated_layers}/{total_layers} layers validated
          </span>
        ...
        {rejected_layers > 0 && (
          <p className="text-xs text-red-600 mt-1.5 font-medium">
            ⚠ {rejected_layers} layer(s) rejected
          </p>
        )}
        {is_complete && (
          <p className="text-xs text-green-600 mt-1.5 font-medium">
            ✓ All layers validated — ready to finalize
          </p>
        )}
      </div>
...
const LayerDetailCard: React.FC<{
  ...
  const overallSurvivabilityDecision = asStr(
    layerData.overall_survivability_decision,
  );
        ...
        <div className="px-5 py-4 space-y-3 bg-white">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {mitigationRequired.map((m, i) => (
                      <span
                        key={i}
                        className="px-2 py-0.5 bg-orange-50 text-orange-700 rounded text-xs font-medium border border-orange-200"
                      >
                        {m.replace(/_/g, " ")}
                      </span>
                    ))}
          ...
          {layerKey === "boundary_verification" && boundaryComplianceStatus && (
            <div className="flex items-center gap-3">
              ...
              <span
                className={`px-2 py-0.5 rounded text-xs font-bold ${
                  boundaryComplianceStatus === "FULLY_COMPLIANT"
                    ? "bg-green-100 text-green-700"
                    : boundaryComplianceStatus === "PARTIALLY_COMPLIANT"
                      ? "bg-yellow-100 text-yellow-700"
                      : "bg-red-100 text-red-700"
                }`}
              >
            ...
            <>
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {soilAcceptance && (
                      <div className="bg-gray-50 rounded-lg p-2 text-center">
                        ...
                        <span
                          className={`text-xs font-bold ${soilAcceptance === "ACCEPT" ? "text-green-600" : "text-red-600"}`}
                        >
                          {soilAcceptance}
                        </span>
                      </div>
                    ...
                    {waterAcceptance && (
                      <div className="bg-gray-50 rounded-lg p-2 text-center">
                        ...
                        <span
                          className={`text-xs font-bold ${waterAcceptance === "ACCEPT" ? "text-green-600" : "text-red-600"}`}
                        >
                          {waterAcceptance}
                        </span>
                      </div>
                    ...
                    {competitionAcceptance && (
                      <div className="bg-gray-50 rounded-lg p-2 text-center">
                        ...
                        <span
                          className={`text-xs font-bold ${competitionAcceptance === "ACCEPT" ? "text-green-600" : "text-red-600"}`}
                        >
                          {competitionAcceptance}
                        </span>
                      </div>
                  ...
                  <div className="space-y-1.5">
                    {soilSuitabilityLevel && (
                      <div className="flex justify-between items-center">
                        ...
                        <span
                          className={`px-2 py-0.5 rounded text-xs font-bold ${
                            soilSuitabilityLevel === "HIGH"
                              ? "bg-green-100 text-green-700"
                              : soilSuitabilityLevel === "MODERATE"
                                ? "bg-yellow-100 text-yellow-700"
                                : "bg-red-100 text-red-700"
                          }`}
                        >
                    ...
                    {waterSuitabilityLevel && (
                      <div className="flex justify-between items-center">
                        ...
                        <span
                          className={`px-2 py-0.5 rounded text-xs font-bold ${
                            waterSuitabilityLevel === "ADEQUATE" ||
                            waterSuitabilityLevel === "HIGH"
                              ? "bg-green-100 text-green-700"
              ...
              {overallSurvivabilityDecision && (
                <div className="flex items-center gap-3">
                  ...
                  <span
                    className={`px-2 py-0.5 rounded text-xs font-bold ${
                      overallSurvivabilityDecision === "ACCEPT"
                        ? "bg-green-100 text-green-700"
                        : overallSurvivabilityDecision === "REJECT"
                          ? "bg-red-100 text-red-700"
                          : "bg-yellow-100 text-yellow-700"
                    }`}
                  >
                    {overallSurvivabilityDecision.replace(/_/g, " ")}
                  </span>
                </div>
              )}
            </>
          ...
          {validatedBy && (
            <p className="text-xs text-gray-400 pt-2 border-t border-gray-50">
              Validated by{" "}
              <span className="font-medium text-gray-600">{validatedBy}</span>
              {validatedAt &&
...
const SpeciesRecommendationsPanel: React.FC<{
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await fetch(
        `http://127.0.0.1:8000/api/update_species/${siteId}/`,
        {
          method: "POST",
          headers: {
...
export default function SiteInformation(): JSX.Element {
  ...
  const fetchSiteData = async () => {
    ...
    try {
      const response = await fetch(
        `http://127.0.0.1:8000/api/get_site/${resolvedId}/`,
        {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
  ...
  const activeMcda: SiteData =
    Object.keys(siteData.validation_data).length > 0
      ? siteData.validation_data
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <div className="flex flex-wrap items-center gap-3 mb-1.5">
              ...
              <span
                className={`px-3 py-1 rounded-full text-xs font-semibold border capitalize ${STATUS_COLORS[siteData.status] ?? STATUS_COLORS.pending}`}
              >
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            ...
            <div className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm">
              ...
              <p className="text-xl font-bold text-gray-800">
                {validationProgress.validated_layers}/
                {validationProgress.total_layers}
              </p>
            ...
            <div className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm col-span-2 md:col-span-1">
              ...
              <p className="text-xl font-bold text-gray-800">
                {siteData.species_recommendations.length}
              </p>
              ...
              {LAYERS.map((layerKey) => {
                ...
                if (!layerData || Object.keys(layerData).length === 0) {
                  return (
                    <div
                      key={layerKey}
                      className="border border-dashed border-gray-200 rounded-xl px-5 py-3 flex items-center gap-3"
                    >
          ...
          >
            ...
            <div className="w-full bg-white/20 rounded-full h-1.5 mb-2">
              <div
                ...
                style={{
                  width: `${(validationProgress.validated_layers / validationProgress.total_layers) * 100}%`,
                }}
              />
            </div>
