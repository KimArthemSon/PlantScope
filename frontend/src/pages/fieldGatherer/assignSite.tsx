...
export default function NDVIMap() {
  ...
  const [form, setForm] = useState<ReforestationArea>({
    name: "",
    legality: true,
    safety: "moderate",
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      // If you want to send image, use FormData
      const formData = new FormData();
      formData.append("name", form.name);
      formData.append("description", form.description);
      formData.append("location", form.location);
      formData.append("legality", String(form.legality));
      ...
      formData.append(
        "polygon_coordinate",
        form.polygon_coordinate
          ? JSON.stringify(form.polygon_coordinate.coordinates)
          : "",
      );

      if (form.area_img instanceof File) {
      ...
      if (!res.ok) {
        console.error("Error creating reforestation area:", data);
        alert("Failed to create area: " + data.detail || "Unknown error");
        return;
      }

      console.log("Reforestation area created:", data);
      ...
      // Optionally refresh the map with new areas
    } catch (err) {
      console.error(err);
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setForm({
                    ...form,
                    area_img: e.target.files?.[0]?.name || null,
                  })
                }
                className="w-full text-[.7rem] mt-1 p-1 border rounded-md focus:ring-2 focus:ring-green-500"
              />
                      ...
                      <img
                        src={
                          typeof area.area_img === "string"
                            ? area.area_img // URL string from server
